...
def findLocalPath(ref_path, status_msg, path_frame, local_path_N): ## global_path와 차량의 status_msg를 이용해 현재waypoint와 local_path를 생성 ##
    ...
    out_path=Path()
    current_x=status_msg.pose_x # posevel
    current_y=status_msg.pose_y
    current_waypoint=0
    min_dis=float('inf')

    for i in range(len(ref_path.poses)) :
        dx=current_x - ref_path.poses[i].pose.position.x # poseStamped
...
class purePursuit : ## purePursuit 알고리즘 적용 ##
    ...
    def getPath(self,msg):
        self.path=msg  #nav_msgs/Path (posestamped)
    
    def getEgoStatus(self, msg): #(posevel)
        #ego_current_velocity = msg.velocity
        # vehicle_yaw = self.convertQuat2Rad(msg) ## orientation 어떻게 구할까??

        self.current_vel = msg.velocity # m/s
        self.vehicle_yaw = msg.yaw   # rad
    ...
    def steering_angle(self): ## purePursuit 알고리즘을 이용한 Steering 계산 ## 
        ...
        for i in self.path.poses:
            path_point=i.pose.position # posestamped
